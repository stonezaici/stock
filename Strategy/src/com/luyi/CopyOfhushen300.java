...
public class CopyOfhushen300 extends JFrame {
	...
	public static void main(String[] args) throws ParseException {
		...
		//下面是画图的定义
		  ...
		  final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	        final Calendar cal = Calendar.getInstance();
		     ...
		     final String sts[]=new String[1500];//这是什么---------start_times
		     final String ets[]=new String[1500];//这是什么---------end_times
		     final String At[]=new String[1500];
		     final String Bt[]=new String[1500];
		     final String Ct[]=new String[1500];
		     final String Dt[]=new String[1500];
		     final  String Mt[]=new String[1500];
		     final String Ap[]=new String[1500];
		     final String Bp[]=new String[1500];
		     final String Cp[]=new String[1500];
		     final String Dp[]=new String[1500];
		     final String Mp[]=new String[1500];
		     final String AAt[]=new String[1500];
		     final String BBt[]=new String[1500];
		     final  String CCt[]=new String[1500];
		     final  String DDt[]=new String[1500];
		     final  String AAp[]=new String[1500];
		     final  String BBp[]=new String[1500];
		     final String CCp[]=new String[1500];
		     final String DDp[]=new String[1500];
		     final String abcStock[]=new String[1500];
		     final String abcIndex[]=new String[1500];
		     final String bcdStock[]=new String[1500];
		     final String bcdIndex[]=new String[1500];
		     final String bcmStock[]=new String[1500];
		     final String bcmIndex[]=new String[1500];
		     final String SabcStock[]=new String[1500];
		     final  String SabcIndex[]=new String[1500];
		     final String SbcdStock[]=new String[1500];
		     final String SbcdIndex[]=new String[1500];
		...
		float bmark=0;//
		float sellmark=0;
		File namefile=new File("D:\\Program Files (x86)\\T0002\\test\\") ;
...
//		int zhiyingCount=0; //止盈交易数
	...
	//以下是创建输出数据
		...
		try{	
			...
			FileReader filereader=new FileReader("D:\\Program Files (x86)\\T0002\\test\\SZ399300.TXT");//上证50指数数据的存储文件
	...
	//		循环读取每一个股票文件然后将相关数据与上证指数相比较
		for(i=0;i<sname.length;i++)	{
			...
			//	System.out.println(sname[i]);
				FileReader filereader=new FileReader("D:\\Program Files (x86)\\T0002\\test\\"+sname[i]);
				BufferedReader reader=new BufferedReader(filereader);
...
//				d=Float.valueOf(price[(int)dt][5]);
			...
			try{
					...
					for(k=0;k<=sell.size()-1;k++){
						if(sell.get(k)[4].compareTo(buy.get(j)[11])>0){//卖出的A'日期必须在M后面，才算买和卖配对成功
							sellmark=k;
//							System.out.println(sell.get(k)[5]+"是否大于"+buy.get(j)[5]);
							System.out.println(sell.get(k)[4]+"是否大于"+buy.get(j)[11]);
							break;
						}
					}//括号：卖出的这层循环结束
			//		System.out.println(sellmark);
					...
					if(k==0) {//k=0表明只买入，未卖出，继续持有   卖出形态没有形成    最后的结果是止盈卖出 和 止损卖出
						...
						for(;price[h][0]!=null && price[h+1][0]!=null && price[h+2][0]!=null && price[h+3][0]!=null;h++){
							...
							/*if(Float.valueOf(price[h][3])>push) {//不是最低点，但是大于push     止盈卖出部分
...
//								ets[seriesNCount]=price[h][1].substring(0,4)+"-"+price[h][1].substring(4,6)+"-"+price[h][1].substring(6,8);
			...
			//			write.newLine();
				...
				//				write.newLine();
					...
					write.write(Float.toString((Float.valueOf(sell.get(k)[3])-Float.valueOf(buy.get(j)[12]))/Float.valueOf(buy.get(j)[12])-fee));//计算卖出后的收益
					System.out.println("配对成功股票代码：" + sname[i]);
					System.out.println("卖出收盘价：" + Float.valueOf(sell.get(k)[3]));
					System.out.println("买入收盘价：" + Float.valueOf(buy.get(j)[12]));
					System.out.println("费用：" + fee);
					write.write("	");
					normalCount++;
					normalRet=normalRet+(Float.valueOf(sell.get(k)[3])-Float.valueOf(buy.get(j)[12]))/(Float.valueOf(buy.get(j)[12])-fee);
					if((Float.valueOf(sell.get(k)[3])-Float.valueOf(buy.get(j)[12]))/(Float.valueOf(buy.get(j)[12])-fee)>0)//里面算的是盈率吗？？？？？？盈率大于0
					{
						winCount++;
						avgWin=avgWin+(Float.valueOf(sell.get(k)[3])-Float.valueOf(buy.get(j)[12]))/(Float.valueOf(buy.get(j)[12])-fee);
					}
					else
					{
						lossCount++;
						avgLoss=avgLoss+(Float.valueOf(sell.get(k)[3])-Float.valueOf(buy.get(j)[12]))/(Float.valueOf(buy.get(j)[12])-fee);
					}
	...
	try {
		...
		BufferedWriter write = new BufferedWriter(osw);	
		write.newLine();
		write.write("正常卖出的平均收益是"+(normalRet/normalCount)+"	");
		write.newLine();
		write.write("总交易次数:"+totalCount + "	");
		write.newLine();
		write.write("正常交易中胜率:"+((float)winCount/normalCount) + "	");
		write.newLine();
		...
		write.newLine();
		write.write("所有交易的平均盈率:"+(-avgWin/avgLoss) + "	");
		write.newLine();
