...
public class hushen300 extends JFrame {
	...
	public static void main(String[] args) throws ParseException {
		...
		float bmark=0;//
		float sellmark=0;
		File namefile=new File("D:\\Program Files (x86)\\T0002\\export_7-1\\") ;
...
//		int zhiyingCount=0; //止盈交易数
	...
	//以下是创建输出数据
		...
		fileout =new File("D:\\newwirthoutput\\"+"hushen日线—"+nowdate+".txt");
		if (!fileout.exists()) {             
			try {
		...
		try{	
			...
			FileReader filereader=new FileReader("D:\\Program Files (x86)\\T0002\\export_7-1\\SZ399300.TXT");//上证50指数数据的存储文件
	...
	//		循环读取每一个股票文件然后将相关数据与上证指数相比较
		for(i=0;i<sname.length;i++)	{
			...
			//	System.out.println(sname[i]);
				FileReader filereader=new FileReader("D:\\Program Files (x86)\\T0002\\export_7-1\\"+sname[i]);
				BufferedReader reader=new BufferedReader(filereader);
