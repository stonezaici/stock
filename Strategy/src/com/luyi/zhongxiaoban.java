...
public class zhongxiaoban extends JFrame {
	...
	public static void main(String[] args) throws ParseException {
		...
		float bmark=0;//
		float sellmark=0;
		File namefile=new File("D:\\Program Files (x86)\\T0002\\export2\\") ;
	...
	//以下是创建输出数据
		...
		fileout =new File("D:\\newwirthoutput\\"+"中小板日线—"+nowdate+".txt");
		if (!fileout.exists()) {             
			try {
		...
		try{	
			...
			FileReader filereader=new FileReader("D:\\Program Files (x86)\\T0002\\export2\\SZ395003.TXT");//上证50指数数据的存储文件
	...
	//		循环读取每一个股票文件然后将相关数据与上证指数相比较
		for(i=0;i<sname.length;i++)	{
			...
			//	System.out.println(sname[i]);
				FileReader filereader=new FileReader("D:\\Program Files (x86)\\T0002\\export2\\"+sname[i]);
				BufferedReader reader=new BufferedReader(filereader);
			...
			for(j=10;price[j+10][1]!=null;j++){//高点的定义为高于或等于前后5根k线的最高点
					//		System.out.prjntln(j);
				//找到高点，并将高点存入lowt数组
				if(Float.valueOf(price[j][5])>=Float.valueOf(price[j-1][5])
									&&Float.valueOf(price[j][5])>=Float.valueOf(price[j-2][5])
			...
			for(j=1;j<hight.size();j++){//当高点之间相差k线根数小于5时，不要前一个高点，用于防止最大值相等是，存在多个高点的情况
			...
			for(j=0;j<hight.size()-1;j++){//找买入形态
				...
				if(sname[i].substring(0, 3).compareTo("SZ002")==0 || sname[i].substring(0, 3).compareTo("SZ002")==0){
